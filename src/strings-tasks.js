...
function formatTime(minutes, seconds) {
  const min = minutes ? minutes.toString().padStart(2, '0') : '00';
  const sec = seconds ? seconds.toString().padStart(2, '0') : '00';
  return `${min}:${sec}`;
}
...
function isPalindrome(str) {
  ...
  return str1 ? str1.split('').reverse().join('') === str1 : false;
}
...
function findLongestWord(sentence) {
  const words = sentence.split(/\b/).sort((a, b) => b.length - a.length);
  return words[0];
}
...
function reverseWords(str) {
  ...
  const reversed = words.map((item) => item.split('').reverse().join(''));
